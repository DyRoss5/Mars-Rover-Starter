...
it("response returned by receiveMessage contains the name of the message", function() {
  ...
  it("response returned by receiveMessage includes two results if two commands are sent in the meassage", function() {
    let commands = [new Command('MODE_CHANGE', 'LOW_POWER'), new Command('STATUS_CHECK')];
    let message = new Message('Test message with two commands', commands);
    let rover = new Rover(98382);  
    let response = rover.receiveMessage(message);
  ...
  it("responds correctly to the mode change command", function() {
    ...
    let message = new Message('Changing to low power mode', commands);
    let rover = new Rover(98382);  
    let response = rover.receiveMessage(message); 
  ...
  it("responds with a false completed value when attempting to move in LOW_POWER mode", function() {
    ...
    let message = new Message('Low Power, cannot be moved', commands);
    let rover = new Rover(98382);   
    let response = rover.receiveMessage(message); 
